# -*- coding: utf-8 -*-
import torch
import torch.nn as nn
import torch.nn.functional as F

# +
# class CNNActionValue(nn.Module):
#     def __init__(self, state_dim, action_dim, activation=F.relu):
#         super(CNNActionValue, self).__init__()
#         self.conv1 = nn.Conv2d(state_dim, 16, kernel_size=8, stride=4)  # [N, 4, 84, 84] -> [N, 16, 20, 20]
#         self.conv2 = nn.Conv2d(16, 32, kernel_size=4, stride=2)  # [N, 16, 20, 20] -> [N, 32, 9, 9]
#         self.in_features = 32 * 9 * 9
#         self.fc1 = nn.Linear(self.in_features, 256)
#         self.fc2 = nn.Linear(256, action_dim)
#         self.activation = activation

#     def forward(self, x):
#         x = F.relu(self.conv1(x))
#         x = F.relu(self.conv2(x))
#         x = x.view((-1, self.in_features))
#         x = self.fc1(x)
#         x = self.fc2(x)
#         return x

class CNNActionValue(nn.Module):
    def __init__(self, state_dim, action_dim, activation=F.relu):
        super(CNNActionValue, self).__init__()
        self.activation = activation

        self.conv1 = nn.Conv2d(state_dim, 16, kernel_size=8, stride=4)  # [N, 4, 84, 84] -> [N, 16, 20, 20]
        self.conv2 = nn.Conv2d(16, 32, kernel_size=4, stride=2)         # [N, 16, 20, 20] -> [N, 32, 9, 9]

        self.in_features = 32 * 9 * 9  # Flatten size

        # Dueling 구조 분리 (Advantage, Value stream)
        self.fc_advantage = nn.Sequential(
            nn.Linear(self.in_features, 256),
            nn.ReLU(),
            nn.Linear(256, action_dim)
        )

        self.fc_value = nn.Sequential(
            nn.Linear(self.in_features, 256),
            nn.ReLU(),
            nn.Linear(256, 1)
        )

    def forward(self, x):
        x = self.activation(self.conv1(x))
        x = self.activation(self.conv2(x))
        x = x.view(x.size(0), -1)  # Flatten

        advantage = self.fc_advantage(x)  # [batch_size, action_dim]
        value = self.fc_value(x)           # [batch_size, 1]

        q = value + (advantage - advantage.mean(dim=1, keepdim=True))
        return q


# class CNNActionValue(nn.Module):
#     def __init__(self, state_dim, action_dim, activation=F.relu):
#         super(CNNActionValue, self).__init__()
#         self.activation = activation

#         self.conv1 = nn.Conv2d(state_dim, 32, kernel_size=8, stride=4)   # [N, 4, 84, 84] -> [N, 32, 20, 20]
#         self.conv2 = nn.Conv2d(32, 64, kernel_size=4, stride=2)          # [N, 32, 20, 20] -> [N, 64, 9, 9]
#         self.conv3 = nn.Conv2d(64, 64, kernel_size=3, stride=1)          # [N, 64, 9, 9] -> [N, 64, 7, 7]

#         self.in_features = 64 * 7 * 7  # Conv3 출력 크기
#         self.fc1 = nn.Linear(self.in_features, 512)
#         self.fc2 = nn.Linear(512, action_dim)

#     def forward(self, x):
#         x = self.activation(self.conv1(x))
#         x = self.activation(self.conv2(x))
#         x = self.activation(self.conv3(x))
#         x = x.view(-1, self.in_features)
#         x = self.activation(self.fc1(x))
#         x = self.fc2(x)
#         return x

# class CNNActionValue(nn.Module):
#     def __init__(self, state_dim, action_dim, activation=F.relu):
#         super(CNNActionValue, self).__init__()
#         self.activation = activation

#         self.conv1 = nn.Conv2d(state_dim, 32, kernel_size=8, stride=4)
#         self.conv2 = nn.Conv2d(32, 64, kernel_size=4, stride=2)
#         self.conv3 = nn.Conv2d(64, 64, kernel_size=3, stride=1)

#         self.in_features = 64 * 7 * 7  # Conv3 출력 크기

#         # Dueling 
#         self.fc_advantage = nn.Sequential(
#             nn.Linear(self.in_features, 512),
#             nn.ReLU(),
#             nn.Linear(512, action_dim)
#         )

#         self.fc_value = nn.Sequential(
#             nn.Linear(self.in_features, 512),
#             nn.ReLU(),
#             nn.Linear(512, 1)
#         )

#     def forward(self, x):
#         # Conv layers
#         x = self.activation(self.conv1(x))
#         x = self.activation(self.conv2(x))
#         x = self.activation(self.conv3(x))
#         x = x.view(x.size(0), -1)  # flatten

#         # Advantage and Value streams
#         advantage = self.fc_advantage(x)  # [batch_size, action_dim]
#         value = self.fc_value(x)           # [batch_size, 1]

#         # Combine them into Q-values
#         q = value + (advantage - advantage.mean(dim=1, keepdim=True))
#         return q
